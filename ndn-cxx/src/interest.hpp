...
public:
  const
  int&
  getInterestSignalFlag() const
  {
    return InterestSignalFlag;
  }

  Interest&
  setInterestSignalFlag(const int& i)
  {
    InterestSignalFlag = i;
    m_wire.reset();
    return *this;
  }
...
public:
  const
  std::string&
  getInterestPITList() const
  {
    return InterestPITList;
  }

  Interest&
  setInterestPITList(const std::string& str)
  {
    InterestPITList = str;
    m_wire.reset();
    return *this;
