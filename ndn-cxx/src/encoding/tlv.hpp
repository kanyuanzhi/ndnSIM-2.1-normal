...
enum ContentTypeValue {
  ...
  /** @brief indicates content is another name which identifies actual data
   * content
   */
  ContentType_Link = 1,
...
inline bool readVarNumber( InputIterator &begin, const InputIterator &end,
                           uint64_t &number );
 ...
 * @throws This call never throws exception
 *
 * This call is largely equivalent to tlv::readVarNumber, but exception will be
 * thrown if type
 * is larger than 2^32-1 (type in this library is implemented as uint32_t)
 */
template <class InputIterator>
inline bool readType( InputIterator &begin, const InputIterator &end,
                      uint32_t &type );

/**
 * @brief Read VAR-NUMBER in NDN-TLV encoding
 *
 * @throws This call will throw ndn::tlv::Error (aka std::runtime_error) if
 * number cannot be read
 *
 * Note that after call finished, begin will point to the first byte after the
 * read VAR-NUMBER
 */
template <class InputIterator>
inline uint64_t readVarNumber( InputIterator &begin, const InputIterator &end );
 ...
 */
template <class InputIterator>
inline uint32_t readType( InputIterator &begin, const InputIterator &end );

/**
 * @brief Get number of bytes necessary to hold value of VAR-NUMBER
 */
inline size_t sizeOfVarNumber( uint64_t varNumber );

/**
 * @brief Write VAR-NUMBER to the specified stream
 */
inline size_t writeVarNumber( std::ostream &os, uint64_t varNumber );

/**
 * @brief Read nonNegativeInteger in NDN-TLV encoding
 *
 * This call will throw ndn::tlv::Error (aka std::runtime_error) if number
 * cannot be read
 *
 * Note that after call finished, begin will point to the first byte after the
 * read VAR-NUMBER
 *
 * How many bytes will be read is directly controlled by the size parameter,
 * which can be either
 * 1, 2, 4, or 8.  If the value of size is different, then an exception will be
 * thrown.
 */
template <class InputIterator>
inline uint64_t readNonNegativeInteger( size_t size, InputIterator &begin,
                                        const InputIterator &end );

/**
 * @brief Get number of bytes necessary to hold value of nonNegativeInteger
 */
inline size_t sizeOfNonNegativeInteger( uint64_t varNumber );

/**
 * @brief Write nonNegativeInteger to the specified stream
 */
inline size_t writeNonNegativeInteger( std::ostream &os, uint64_t varNumber );
...
template <class InputIterator>
inline uint64_t readVarNumber( InputIterator &      begin,
                               const InputIterator &end ) {
  if ( begin == end )
...
template <>
inline bool readVarNumber<std::istream_iterator<uint8_t>>(
    std::istream_iterator<uint8_t> &      begin,
    const std::istream_iterator<uint8_t> &end, uint64_t &value ) {
  if ( begin == end ) return false;
...
inline uint64_t readNonNegativeInteger( size_t size, InputIterator &begin,
  ...
  case 1: {
    if ( end - begin < 1 )
      BOOST_THROW_EXCEPTION(
          Error( "Insufficient data during TLV processing" ) );
  ...
  BOOST_THROW_EXCEPTION( Error( "Invalid length for nonNegativeInteger (only "
                                "1, 2, 4, and 8 are allowed)" ) );
}

template <>
inline uint64_t readNonNegativeInteger<std::istream_iterator<uint8_t>>(
    size_t size, std::istream_iterator<uint8_t> &begin,
